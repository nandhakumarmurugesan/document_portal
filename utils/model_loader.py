...
class ModelLoader:
    ...
    def load_embeddings(self):
        """
        Load embeddings and return the embeddings model
        """
        try:
    ...
    def load_llm(self):
        ...
        """
        #Load llm dynamically based on provider in config loader
        llm_block = self.config["llm"]
        log.info("Loading LLM model from config")
        ...
        temperature = llm_config.get("temperature", 0.2)  # default temperature
        max_tokens = llm_config.get("max_output_tokens", 2048)  # default max tokens
